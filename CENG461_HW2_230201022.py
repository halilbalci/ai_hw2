...
"""----------------------------------------------------------------------------------------------------------------------"""
def app():
    query=input("please give query variables('quit' to quit):")
    if(query=="quit"):
        sys.exit()
...
def calculateByInference(numerator_string,denominator_string):
    return bayesNetwork(numerator_string)/bayesNetwork(denominator_string)
    
def bayesNetwork(string):
    result=0
...
while loop:
    try:
        ...
        print("The denominator is 0. The probability calculation by ınference can not be done.")
    except:
